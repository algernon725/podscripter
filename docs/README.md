...
`podscripter` is a lightweight tool designed to transcribe audio using OpenAI's Whisper model inside a Docker container. It supports multiple languages with automatic language detection, including English (`en`), Spanish (`es`), French (`fr`), and Portuguese (`pt`). 
...
- Docker-compatible system (Mac, Linux, Windows with WSL). Podman works too, and is the recommended runtime on Bazzite and other Fedora Atomic distributions.
