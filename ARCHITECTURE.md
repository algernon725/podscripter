...
- 33 test invocations across 5 files are marked `@pytest.mark.xfail` (per-parameter on parametrized tests). These are all verified NLP model limitations: 31 for sentence splitting/formatting drift and 2 for inverted question-mark fusion cleanup. The 50 implicit/verb-first question-detection xfails were retired in v0.10.1 when that issue was closed as an accepted limitation (production relies on Whisper's native punctuation; text-only detection of verb-first questions is inherently ambiguous). Low-value xfail tests (run-on detection, initials, numbers, whisper boundaries) were removed in v0.8.2. See AGENTS.md for root causes and rationale.

### Real-audio EN/ES/FR test corpus (three tiers)

- **Tier 1 — regression** (`tests/test_audio_fixtures.py`, `@pytest.mark.transcription`): per-clip `.expected.json` metadata under `tests/fixtures/audio/<lang>/` (currently `en/`, `es/`, `fr/`) paired with audio hosted in the public HuggingFace dataset `podscripter-project/test-fixtures`, pinned to a specific commit revision in `tests/fixtures/audio/download.py`. Each fixture runs through the full pipeline (`audio -> Whisper -> pyannote -> SentenceSplitter -> SentenceFormatter -> writer`) with the same flags as typical manual usage (`enable_diarization=True`, `model_name="medium"`, `beam_size=3`, `single_call=True`). Long fixtures (with `"modes": ["single", "chunked"]`) also exercise `_split_audio_with_overlap` / `_dedupe_segments` / `_accumulate_segments`. Asserted thresholds: WER (via `jiwer`) and DER (via `pyannote.metrics`) with per-fixture loose bounds that absorb pyannote/Whisper non-determinism. Spanish fixtures additionally exercise the `spanish-questions` pattern (requires both `¿` and `?` in output). All three primary languages have one long multi-speaker MLS-concat fixture exercising both `single` and `chunked` modes (`en/librispeech_two_speakers_long.flac`, `es/mls_es_two_speakers_long.flac`, `fr/mls_fr_two_speakers_long.flac`).
- **Tier 2 — quality benchmark** (`tests/benchmarks/`): runs the pipeline over bounded public dataset subsets and tracks **WER** per clip against a committed, populated `tests/benchmarks/baseline.json` (75 clips: 15 each of FLEURS en_us/es_419/fr_fr + MLS Spanish/French, `medium` model). FLEURS is pulled as its per-language test archive; MLS is *streamed* clip-by-clip from `facebook/multilingual_librispeech` (raw `.opus` + `transcripts.tsv`, bounded by `--mls-max-items`) rather than downloaded as the multi-GB monolithic archive. WER-only by design — every Tier 2 dataset is single-speaker, so DER is left to the Tier 1 multi-speaker fixtures (`compare_baseline.py` will still diff a `der` field if one is added later). The baseline was populated by running the benchmark twice on a clean image and confirming bit-identical per-clip WER (decoding is deterministic), then committing the second run. **Not run per-PR and not in CI** — it is a manual, ad-hoc drift gate run deliberately after a transcription/model change; refresh `baseline.json` in the same change when intentionally accepting drift. Optional future axes (DER + multi-speaker `chunked` mode, VoxPopuli/LibriSpeech enumeration, nightly CI) are deferred — see the `Tier 2 benchmark — remaining optional axes` entry in `AGENTS.md` §6.
- **Tier 3 — bug fixtures**: trimmed clips of real failures live in the same HF dataset under `<lang>/bug_<name>.<ext>`, paired with focused regression tests mirroring `tests/test_episodio272_speaker_split_exclamation.py`. The `HF_REVISION` bump lands in the same PR as the metadata + test so the audio and tests stay in lockstep across `git checkout`s.
- **Storage approach**: the podscripter repo stays binary-free (matches the existing `.gitignore` policy for `audio-files/` and `models/`); audio fixtures live exclusively in the HF dataset and are downloaded into the same `HF_HOME` cache as Whisper + pyannote models. License compliance (CC-BY 4.0 / CC0 only) is enforced by `tests/fixtures/audio/_validate_licensing.py`, which runs on every `pytest` invocation under the default `core` marker.
...
2. Grammatical guards (never break on conjunctions/prepositions/auxiliary verbs)
   ...
   - **v0.10.7**: Extended the Spanish `spanish_forbidden` set in `_violates_grammatical_rules()` with words that can *never* end a sentence: proclitic object/reflexive pronouns (`me`, `te`, `se`, `nos`, `os`, `le`, `les`, `lo` — attach forward to the verb, prevents "me. Imagino") and apocopated prenominal adjectives (`buen`, `gran`, `primer`, `tercer`, `algún`, `san` — exist only before a noun, prevents "un buen. Apartamento"). `mal` deliberately excluded (also an adverb that can end a sentence: "me trata mal").
...
5. Semantic coherence (fallback)
   ...
   - **v0.10.7**: Grammatical guards on the *semantic split point* — before running the semantic model, suppress splits that would sever tightly-bound constructions on a long single-speaker run (where no Whisper/speaker boundary exists). These are scoped to the semantic path (unlike PRIORITY 2, which is language-general never-break) because each construction *can* legitimately sit at a sentence edge in other contexts: comparative particles both edges (`un poco | más | baratos`, es/en/fr/de — `_is_bound_comparative_break()`), the clause-hinge `que` both edges (`imagino | que | puedes` — `SEMANTIC_BREAK_HINGE_WORDS`), modal + governed infinitive (`puedes | encontrar` — `_is_modal_infinitive_break()`), and infinitive + complement (`encontrar | un buen apartamento` — `_is_infinitive_complement_break()`). Conditional guards fire only when the following word is lowercase (a genuinely sentence-final token is followed by a capitalized word). Discovered peeling one long run-on sentence in `Episodio300.mp3`.
