...
### Real-audio EN/FR test corpus (three tiers)

- **Tier 1 — regression** (`tests/test_audio_fixtures.py`, `@pytest.mark.transcription`): per-clip `.expected.json` metadata under `tests/fixtures/audio/<lang>/` paired with audio hosted in the public HuggingFace dataset `podscripter-project/test-fixtures`, pinned to a specific commit revision in `tests/fixtures/audio/download.py`. Each fixture runs through the full pipeline (`audio -> Whisper -> pyannote -> SentenceSplitter -> SentenceFormatter -> writer`) with the same flags as typical manual usage (`enable_diarization=True`, `model_name="medium"`, `beam_size=3`, `single_call=True`). Long fixtures (with `"modes": ["single", "chunked"]`) also exercise `_split_audio_with_overlap` / `_dedupe_segments` / `_accumulate_segments`. Asserted thresholds: WER (via `jiwer`) and DER (via `pyannote.metrics`) with per-fixture loose bounds that absorb pyannote/Whisper non-determinism.
- **Tier 2 — quality benchmark** (`tests/benchmarks/`): pulls ~30 min/lang of public dataset subsets (FLEURS, LibriSpeech, optionally VoxPopuli) and tracks WER + DER over time against `tests/benchmarks/baseline.json`. Multi-speaker subsets run in both `--single` and chunked modes so chunked-specific regressions surface separately. Not run per-PR; intended for nightly CI or pre-release validation.
