...
- **Tier 2 — quality benchmark** (`tests/benchmarks/`): pulls ~30 min/lang of public dataset subsets (FLEURS en_us/es_419/fr_fr, LibriSpeech, MLS Spanish, optionally VoxPopuli) and tracks WER + DER over time against `tests/benchmarks/baseline.json`. MLS French wiring + DER metric + populating `baseline.json` itself are deferred to a future Phase 3 effort (see the `Tier 2 baseline.json` DEFERRED entry in `AGENT.md` §6); today the committed baseline is empty and the only Tier 2 metric is WER. Multi-speaker subsets are designed to run in both `--single` and chunked modes so chunked-specific regressions surface separately, but no multi-speaker subset is enumerated today (also deferred). Not run per-PR; intended for nightly CI or pre-release validation.
