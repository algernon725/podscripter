...
### Fixed
- **SentenceTransformer punctuation model made an unauthenticated HF Hub network request on every run despite a warm cache** — during the "Restoring punctuation..." phase, loading `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` in `_load_sentence_transformer` (`punctuation_restorer.py`) emitted `Warning: You are sending unauthenticated requests to the HF Hub...` and performed a network metadata (HEAD) round-trip on every invocation, even though the weights were already cached locally. Reproduced with `python podscripter.py audio-files/Episodio300.mp3 --single --language es --enable-diarization`. This was a long-standing, low-priority annoyance (the weights themselves were cached, so it was a per-run network check rather than a re-download).
  - **Root Cause**: Two independent reasons the existing offline logic never took effect. (1) `huggingface_hub` freezes `HF_HUB_OFFLINE` into a module constant **at import time**; `faster_whisper` (imported at `podscripter.py` startup) pulls in `huggingface_hub` before punctuation runs, so the loader's later `os.environ.setdefault("HF_HUB_OFFLINE", "1")` was a **no-op** — verified: setting the env var after import leaves `huggingface_hub.constants.HF_HUB_OFFLINE == False`. (2) The direct-path offline load checked for `modules.json` at the cache **root** (`.../models--sentence-transformers--.../modules.json`), but that file lives under `snapshots/<hash>/`, so the guard was always false and the code fell through to a name-based load that hit the network. (The `Loading weights: 199/199` tqdm bar is normal `transformers` weight-loading output and appears even when fully offline — it is not a download and was never the problem; only the network HEAD request was.) pyannote diarization was unaffected — the "unauthenticated" warning (emitted once per process on the first offline-unaware request) first appeared at punctuation time, proving the pipeline load made no such request.
  - **Fix**: `_load_sentence_transformer` now detects a warm cache via the existing `_find_local_model_path(...)` and loads with **`local_files_only=True`** (org-qualified name, then short-name fallback), which is honored **per-call** and bypasses the frozen `HF_HUB_OFFLINE` constant — no network request, no warning. A cold/incomplete cache falls through to the prior online load (`local_files_only=False`), preserving first-run downloads. The dead `modules.json` direct-path block was removed (the offline name-based load supersedes it and resolves the Sentence-Transformers snapshot layout correctly, so no "Creating a new one with mean pooling" warning); the ineffective `HF_HUB_OFFLINE` `setdefault` is documented as a no-op.
  - **Result**: warm-cache runs load the punctuation model fully offline with zero HF Hub network requests. Confirmed end-to-end: the `--enable-diarization` run no longer prints the unauthenticated-requests warning; punctuation output is unchanged.
  - **Tests**: verified against the real import order (`faster_whisper` imported first) that the warm-cache load emits no unauthenticated warning and the cold cache correctly routes to the online fallback. Regression: core suite (519 passed, 34 xfailed) plus `tests/test_punctuation.py`, `tests/test_improved_punctuation.py`, `tests/test_punctuation_preservation.py`, `tests/test_environment_variables.py`, `tests/test_no_deprecation_warning.py` (25 passed) — output identical.

### Notes
- **Patch bump (0.10.9)** — narrow, production-visible fix confined to `_load_sentence_transformer`; no threshold/model/behavioral change to transcription output, only a reduction in network calls. General lesson: for any HF-backed model that must load offline from a warm cache, prefer the loader's own `local_files_only=True` argument over setting `HF_HUB_OFFLINE` at runtime, since `huggingface_hub` reads that env var once at import time.
...
### Fixed
- **Ungrammatical sentence breaks from the semantic splitter on long single-speaker run-ons (Spanish)** — on a chunk long enough to trigger the PRIORITY-5 semantic coherence check (≥42 words for Spanish) with no Whisper/speaker boundary nearby, `SentenceSplitter._check_semantic_break()` chose the split *location* purely from embedding similarity, with no guard against severing tightly-bound constructions. Reproduced across two sentences in `audio-files/Episodio300.txt` for `Episodio300.mp3` (`--single --language es --enable-diarization`), e.g. `"…los costos son un poco."` / `"Más baratos para…"` and, on a genuinely one-sentence 40+ word run, a break that hopped word-by-word (`me | imagino`, `imagino | que`, `que | puedes`, `puedes | encontrar`, `encontrar | un`, `un buen | apartamento`) as each edge was guarded. `max_words_force_split` (defined in `_get_language_thresholds`) was confirmed **dead code** (never referenced) and plays no role.
  - **Root Cause**: The semantic heuristic is the lowest-confidence signal and had no grammatical constraints on *where* it may break. Higher-priority signals were behaving correctly (two upstream Whisper boundaries were legitimately suppressed by the same-speaker connector guard), which grew the chunk past the semantic threshold; the model then broke at a linguistically invalid point. A word that binds in **both** directions (comparative particle, subordinating `que`) required guarding both edges — guarding one edge just moved the break to the other.
  - **Fix**: Added grammatical guards at two priority levels in `sentence_splitter.py`:
    - **PRIORITY 2 (general never-break, `_violates_grammatical_rules()` `spanish_forbidden`)** — proclitic object/reflexive pronouns (`me`, `te`, `se`, `nos`, `os`, `le`, `les`, `lo`; attach forward to the verb) and apocopated prenominal adjectives (`buen`, `gran`, `primer`, `tercer`, `algún`, `san`; exist only before a noun). Both classes can *never* end a sentence. `mal` excluded (also an adverb: "me trata mal").
    - **PRIORITY 5 (semantic path only, conditional)** — comparative particles guarded on **both** edges (`COMPARATIVE_PARTICLES` / `DEGREE_QUANTIFIER_HEADS` / `_is_bound_comparative_break()`, es/en/fr/de: `"un poco | más | baratos"`); clause-hinge `que` on both edges (`SEMANTIC_BREAK_HINGE_WORDS`); modal + governed infinitive (`INFINITIVE_GOVERNING_VERBS` + `_ES_INFINITIVE_RE` / `_is_modal_infinitive_break()`: `"puedes | encontrar"`); infinitive + complement (`_is_infinitive_complement_break()`: `"encontrar | un buen apartamento"`). These are scoped to the semantic path (not the general guard) because each construction *can* legitimately sit at a sentence edge — the conditional guards fire only when the following word is lowercase (a genuinely sentence-final token is followed by a capitalized word).
  - **Result**: segment 70 (`"…me imagino que puedes encontrar un buen apartamento entre trescientos hasta como setecientos dólares al mes."`) is emitted as one complete, grammatical sentence; the comparative sentence keeps `"un poco más baratos"` intact.
  - **Tests**: `tests/test_comparative_split.py` (40) and `tests/test_proclitic_pronoun_split.py` (45) — predicate positives/negatives per class plus splitter-level isolation tests (low-similarity stub model) proving each guard, not text length, suppresses the split while unguarded content-word junctions still break. Full Spanish/splitting/formatter/punctuation sweep: 273 passed, 33 xfailed, no new failures.

### Notes
- **Patch bump (0.10.7)** — narrow, production-visible fix confined to grammatical guards in `SentenceSplitter`; no threshold/model changes. Two design patterns emerged and are documented in `ARCHITECTURE.md` (priority hierarchy, items 2 and 5) and `.agent/troubleshooting/history.md` (six Resolved-bugs entries): (1) a word that binds in both directions needs both edges guarded at once; (2) "never valid" constructs (proclitics, apocopes) go in the general guard, while "conditionally valid" ones (comparatives, modals, infinitives, `que`) are scoped to the low-confidence semantic path and gated on a lowercase continuation.
- **Known gaps (unchanged, not addressed here)**: English synthetic `-er` comparatives (`"a little cheaper"`) are not enumerable and remain unguarded; French proclitic pronouns were intentionally left out (Spanish-only for this fix); the preposition-attachment case (`"…baratos. Para los de…"`) is a separate class (prepositions can legitimately start sentences) and is not addressed.
